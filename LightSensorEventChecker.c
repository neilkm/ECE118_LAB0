...
uint8_t CheckLightEvent(void) {
    static LightState_t lastState = DARK;
    LightState_t curState;
    ...
    if (curState != lastState) {
        ...
        thisEvent.EventParam = (uint16_t)curState;
        returnVal = TRUE;
        lastState = curState;

#ifndef EVENTCHECKER_TEST
        PostLightSensorService(thisEvent);  // or whatever your service is
#else
        SaveEvent(thisEvent);     // for test harness
#endif
    }
